...
if not my_script_path in sys.path:
    print("Adding sript pos cameras :D")
...
if not my_script_path in sys.path:
    print("Adding script perso data")
    sys.path.append(my_script_path)
    
import Data_personnage as perso_pos
...
class DECOR_SMF_IMPORT(bpy.types.Operator):
    ...
    def execute(self,context):
        #pcoll = gestionnaire d'aperçus et ensuite on utilise les icons dans le widget de l'interface
...
#        cam_object.select_set(True)
        # Selectionne selement les cameras de la scène et en fait un fbx    
        for obj in bpy.context.scene.objects:
...
class FileSaveCameras(bpy.types.Operator):
    ...
    #Import la camera selectionnée au format fbx et la nomme avec le nom de la camera
    #On fait rendu du viewport au format png que l'on ajoute à la librairie d'icon pour la banque des cameras
...
class DELETE_CAMERA(bpy.types.Operator):
    ...
    #Supprime l'icon preview de la camera
    #supprime la camera de la scene 
...
class ImportCameras(bpy.types.Operator):
    bl_idname = "test.import_camera"
    bl_label  = "import Camera"
    bl_description = "Import une cameras"
    
    #Va chercher le fbx de la camera et l'import
...
class RenderPngOperator(bpy.types.Operator):
    ...
    #Verifie avec glob.glob dans le dossier toutes les incrémentations du rendu 
    #Ajoute plus un à la liste des versionings 
    #definit le format png
    #on fait le rendu avec l'itteration que l'on va enregistrer dans le dossier 
...
class SimpleOperator(bpy.types.Operator):
    ...
    # poll corespond à une variable bool vrai ou faux 
    @classmethod
...
################################# FOCAL #######################################
#Pour afficher la focal dans la vue 3d et dans la vue camera
...
# class Modal pour rafraichir le string de la focal
...
#    def execute(self, context):
#       #switch bool property to opposite. if you don't toggle just set to False
#        context.scene.my_bool_focal = not context.scene.my_bool_focal
#        self.report({'INFO'}, "Simple Operator executed.")
#        return {'FINISHED'}
...
class NavigationSimplifiee (bpy.types.Operator):
    ...
    #Pour toutes les vue et les fenêtre on peut se ballader en mode walk
    def execute(self,context):
...
class SplitArea(bpy.types.Operator):
    ...
    # verifie si la vue est persp ou camera et separe la fenêtre avec la vue inverse
    def execute(self, context):
...
class JoinArea(bpy.types.Operator):
    ...
    # Mets le view port en une vue full screen
...
class FileSave(bpy.types.Operator):
    ...
    #Fait un pop up de la fenêtre save file pour laisser le choix de où ranger le fihier
...
class VuePersp(bpy.types.Operator):
    ...
    #Passe la vue actuelle en vue Persp
...
class VueCam(bpy.types.Operator):
    ...
    #Passe la vue actuelle en vue Camera
...
class InsertKeyframe(bpy.types.Operator):
    ...
    #Ajoute des keyframes sur loc,Rot et normalement focal mais petit soucis
...
class DeleteKeyframe(bpy.types.Operator):
    ...
    #Delete le keyframes sur location et rotation à l'endroit du cursor
...
class Animation(bpy.types.Operator):
    ...
    #Divise l'écran par le bas pour faire apparaitre la timeline = dopesheet_editor
...
class RenderAnimation(bpy.types.Operator):
    ...
    #Elect le format ffmpeg definit l'emplacement et rend l'animation
...
#        box.prop(context.scene.camera.data, "lens", slider=True)

        row=layout.row()
        if scene.my_bool_property:#if bool property is true, lock grise la focal, else don't
...
class PreviewsCamerasPanel(bpy.types.Panel):
    ...
    def draw(self,context):
        ...
        row.operator("test.import_camera", icon = "IMPORT")
...
#    bpy.types.Scene.my_bool_focal = BoolProperty(name='My Bool Focal', default = True)
    
###################################################################################

bpy.types.Scene.my_bool_property = BoolProperty(name='My Bool Property', default = True)# create bool property for switching focal   
...
#    del bpy.types.Scene.my_bool_focal
